...
| 分类 | 仓库扫描结果 | 迁移动作 |
| --- | --- | --- |
| 已符合 | 统一 DPR 解析真源已区分运行时上限 2 与图片生产基线 2 | 保留并由新合同复用 |
...
7. 历史图片生产 `1.5` 合同及其尺寸/清晰度证据不再视为当前基线：活跃工件必须按生产 DPR `2` 重新生成、重验并更新身份；暂未重验的历史证据必须显式标记 `stale`，不得继续作为 V3/V4 放行依据。该规则不影响运行时代表性 DPR `1.25/1.5` 测量证据的合法性。
