...
控制面边界：可提议、可审查、可在批准范围内记录决定，且必须回到 `$phaser4-game-workflow-control` 审批。

Grilling 只形成明确用户决定，并将用户原文、对象、当前门、阶段、模块、基线、动作和失效条件提交给 [`phaser4-game-workflow-control`](../phaser4-game-workflow-control/SKILL.md) 写 Approval Ledger。它不拥有全局状态或审批，不能把“继续”“可以”或 blanket approval 扩展到后续门。
...
3. 首次模块或边界变化强制进入模块门与 grilling；其他决定只在存在实质用户取舍时进入。记录必须绑定 Work Item、当前门、基线与失效条件，不得虚构候选身份。
4. 无产品定义时从目标用户、首次价值、范围、商业约束和风险开始；已有定义时只追问缺口、冲突、假设或依赖，不重复已有结论。
5. 新阶段或新页面本身不是触发器；首次模块实现或边界变化是全局控制面规定的强制触发器。
6. 视觉方向、预算、签名强度或实现/资产成本仍需取舍时才做开放式视觉拷问。已有明确适用基线时不重复确认。
7. V1、V2 与发布决定按当前 Work Item 的明确门记录；视觉确认只覆盖所列对象，不能授权正式资源或后续阶段。
...
用户明确确认共同理解后，只汇总实际决定、未决项、依赖、拒绝项和下一步。将决定提交给全局控制面写入 `.workflow-control/approvals/ledger.json`，领域负责人再更新对应权威工件；V1/V2 保留各自对象记录。不得记录凭据、个人数据或受限合同全文。
