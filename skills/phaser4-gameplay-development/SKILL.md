...
2. 按 TDD 的 G1 场景 Work Item 序列执行：任务授权/范围与功能规格（只定义契约，不写正式功能代码）→ V1 视觉合同/参考冻结 → 当前场景 V2 完整候选、动态样片、F2 `MACHINE/PASS` 与唯一真人视觉审批 → V3 实施拆解 → V4 正式视觉资源与宿主场景同屏组合预验收 → 正式功能代码实现 → V5 运行态视觉接入与功能/视觉联合验收 → A4 正式入口。全局 `visual_baseline` 只负责静态风格一致性，全局场景集合只作规划/聚合事实；V2 `COMPLETE/frozen` 是正式 A3 包和功能代码的唯一前置视觉边界，V2 前只允许隔离灰盒或无正式业务逻辑视觉样片，不得注册正式入口。V4 之后才按计划制定者冻结的 `SHARED` 最小运行骨架、`MODULE`、实际场景顺序的 `SCENE` 及紧邻从属 `DISPLAY_LAYER`，场景阶段全部完成后才做跨场景 `INTEGRATION`/联合验收。gameplay/supporting 只保留场景分类，首个可玩切片只作为中间里程碑，不停止后续实施。
