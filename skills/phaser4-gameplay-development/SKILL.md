...
2. 按 TDD 的 G1 全局序列先通过全局视觉效果图冻结门（覆盖全部授权 gameplay/supporting 场景的 scene master 与必需瞬态宿主上下文效果图），再实施 `SHARED` 最小运行骨架、`MODULE`，最后按计划制定者冻结的实际场景顺序实施每个 `SCENE` 及其紧邻从属 `DISPLAY_LAYER`，场景阶段全部完成后才做跨场景 `INTEGRATION`/联合验收。骨架不得提前实现完整模块或场景业务；gameplay/supporting 只保留场景分类，首个可玩切片只作为中间里程碑，不停止后续实施。
...
5. 模块实现只能在最小骨架稳定后、场景实现前进行；模块可按互斥文件和状态所有权并行，共享契约/入口保持串行。场景实现必须在模块完成后，把玩法、正式资源、全部 HUD/UI/modal/popup/drawer/toast 显示层、清理和联合证据一起闭环；不得把纯规则或 UI/弹窗拆到宿主场景之外。Work Item 指定效果图为还原目标时，玩法实现必须保留冻结视觉目标及已批准例外，不得以工程便利、提升游戏感或专业修复为由产生未登记的可见偏差；静态图无法证明的玩法、交互或动画仍标为待定义。不得自行裁切合成效果图，也不得把整屏效果图、低保真或关键画面当正式交互资源。
