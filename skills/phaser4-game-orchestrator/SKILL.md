...
V0 分流，V1 契约/低保真，V2 方向/高保真，V3 生产规划，V4 正式资源，V5 运行态集成。普通资产在 schema 1.5 声明还原 `not-applicable`。效果图还原冻结后、V3 前完成合同回对与 coverage，生命周期为 `v3-ready`；拆解顺序固定为“先状态分析，再按可复用部件拆分”：每个 region 的 `state_analysis` 必须覆盖 default、selected、active、disabled、pressed、hover、victory、defeat、paused，并逐项写 `required` 或 `not-applicable+reason`；`annotation_number` 不是资产数量，② 6 个顶部按钮、⑧ 3 个底部表面、⑨ 3 个动作图标必须逐部件登记。`expected_assets` 默认逐部件独立交付；ImageGen 无条件要求独立位图、`delivery_mode=individual`、`atlas_allowed=false`，禁止横向组图和图集；仅 authored-raster/authored-svg/reuse 等非 ImageGen 方法可在显式合同及完整切片元数据下使用 atlas，交互热区不计入视觉资产。之后再依据 ownership/F2 事实在冻结效果图上生成编号 PNG（左原图、右侧说明栏）并展示用户，明确本次生成、复用既有资源和程序实现。只有 `bitmap-decomposition` 的 `generate-now` 区域必须先生成绑定冻结目标 SHA/region ID/区域定义 SHA 的编号提案，在 confirmation 中绑定 proposal/decision 文件及 SHA、PNG/MIME/版本/SHA、decision_id 和实际消息身份后等待一次精确 USER_DECISION；正式流程不生成或接受 SVG 标注。复用和程序实现区域不触发位图确认但必须可见。`reuse-existing` 必须使用不可变 `asset-reuse-snapshot/1.0`，额外精确绑定 `source_file`、`source_manifest_sha256`、`source_sha256`、`compatibility_evidence_sha256`，并由文件检查确认快照资源为 `accepted`；冻结原图须为与目标画布同尺寸的完整合法 PNG。开始任何拆解生产前必须运行带 `--check-files --project-root .` 的校验且结构和文件证据均通过，确认前不得裁切、抠图、分层、AI 分割/补全或生产派生位图；确认只授权拆解范围，不改变玩法、布局或视觉事实。V3/V4 可暂无 fidelity case，只有 V5 验证完成才标记 `v5-complete` 并要求全部通过。owner_type 是合同/F2 专业事实，验证器不从像素推断。
