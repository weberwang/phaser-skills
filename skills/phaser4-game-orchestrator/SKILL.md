...
默认闭环为：`最小必要事实确认 → 冻结候选边界（A3 冻结 Implementation Package）→ 执行/实施 → diff-audit → 获授权的定向验证 → 仅按失败证据修正 → 完成`。仅 A3 包冻结后，implementer 不做开放式方案探索；只有需求/范围变化、实质冲突或无法实施才返回。已确认事实不得重复读取/搜索/复核，除非出现新失败证据、运行异常、直接矛盾、需求/范围明确变化、候选身份实际变化或硬门失败。复核只检查需求覆盖、明确缺陷、越界和硬门，不因另一可行方案推翻满足需求的候选；非阻塞发现记录为未覆盖项/后续事项，不扩大当前 Work Item。详细返工条件见 [统一门](references/review-funnel.md)。
