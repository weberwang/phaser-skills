...
description: 面向 Phaser 4、TypeScript、Vite 与 Capacitor 的移动端 2D 游戏全周期总控。需要在模块实现前组织划分与强制拷问，将游戏从立项推进至小游戏、iOS、Google Play 或扩展渠道的可提审交付，或协调制作策划、技术架构、玩法、数值、美术、音频、测试性能和发布合规角色时使用。
---
...
- 快速通道：已批准模块边界内的局部、低风险、可回退任务。明确验收条件后直接实现和验证；发现需要新建模块或改变模块边界时立即升级标准通道并执行模块拷问门禁。除存在多个并发写入者外，不强制调用角色、建立 worktree、更新控制面或通过 G0 至 G3。
...
确认后将问题、事实、结论、影响和下一步写入 docs/control-plane.md，再更新受影响文档。模块拷问通过后还要在 TDD 将模块标为“已批准待实现”；未处于该状态不得开始生产实现。角色只提交简短决策包给总控，不重复发起拷问。
...
| G1 切片 | 运行 references/game-implementation.md 的垂直切片任务链：冻结实现增量，执行“候选模块图 → 强制拷问 → 人工批准 → 冻结 TDD 契约”，再建立可运行骨架；美术先生成满足“竖屏高度适配、横屏宽度适配”的无交互满幅背景并等待人工确认，架构提供统一适配器，玩法在每个可见场景应用背景；其余重构完成后由测试验证。 | 所有已实现模块均有实现前拷问和人工批准记录，职责、状态所有权、公开契约与依赖方向和实现一致；干净环境可构建；核心循环可完成；正式资源有人工确认记录；竖屏按高度、横屏按宽度适配时无黑边或透明缝；测试、构建和视觉证据可复现；已选渠道至少一个可运行。 |
| G2 制作 | 每个新增或边界变化模块先重复“模块划分 → 强制拷问 → 批准契约 → 实现”，再按已批准验收增量执行“规则实现 ↔ 美术重构/人工确认/切图/资源接入 ↔ 集成验证”闭环；资源未就绪时只允许登记了替换责任人与截止门的占位资源。 | 范围、模块边界与视觉系统冻结；全部批准功能已实现并有验收证据；实现没有未批准模块、契约或依赖；正式图片具有确认、纹理键、视觉对比和接入证据；占位资源已清零；设备与性能证据齐备；P0/P1 已处理或获人工豁免。 |
