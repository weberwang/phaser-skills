...
description: 面向 Phaser 4、TypeScript、Vite 与 Capacitor 的移动端 2D 游戏全周期总控。需要在模块实现前组织划分与强制拷问，根据参考截图、录屏、运行项目或源代码同时还原视觉与功能，将游戏从立项推进至小游戏、iOS、Google Play 或扩展渠道的可提审交付，或协调制作策划、技术架构、玩法、数值、美术、音频、测试性能和发布合规角色时使用。
---
...
- 快速通道：已批准模块边界内的局部、低风险、可回退任务。明确验收条件后直接实现和验证；发现需要新建模块或改变模块边界时立即升级标准通道并执行模块拷问门禁。不强制调用角色、建立 worktree、更新控制面或通过 G0 至 G3；只有人工明确要求时才使用 worktree。
