...
2. 任务授权和范围冻结后，先完成功能规格与玩法契约（只定义行为、状态、输入、验收和边界，不写正式功能代码），再在 V1 冻结视觉合同、参考身份、目标视口/状态、布局和容差。全局 `visual_baseline` 只负责静态风格一致性；覆盖全部授权场景的 scene master 与必需瞬态宿主上下文图属于规划/聚合事实，不能代替逐场景 V2。
3. V2 是当前场景 Work Item 的唯一前置视觉验收：完整场景候选、动态视觉样片、F2 `MACHINE/PASS` 和唯一真人视觉审批必须收敛为同一份 `COMPLETE/frozen` 结果。V2 通过前只允许隔离灰盒或无正式业务逻辑的视觉样片，不得注册正式入口；任何正式 A3 Implementation Package、`SHARED`、`MODULE`、`SCENE` 或 `DISPLAY_LAYER` 功能代码都必须等待 V2。
4. V3 进行实施拆解并冻结 Implementation Package，V4 生产正式视觉资源、完成正式布局与宿主场景同屏组合预验收。参考/效果图还原是当前场景 Work Item 内的可选视觉实现模式，不是独立任务或第二条 V1→V5；适用时其合同、候选、机器验证和真人审批均归入同一 V2 结果。
5. V4 通过后才按计划制定者冻结的 `executionUnits` 顺序启动正式功能实现：`SHARED` 最小运行骨架→`MODULE`→按场景组织的 `SCENE`+紧邻从属 `DISPLAY_LAYER`。每个场景必须把玩法、正式资源、全部 HUD/UI/modal/popup/drawer/toast、生命周期清理和联合证据闭环；显示层必须复核宿主上下文图与 scene/layer/host 身份，并紧邻、归属于宿主 `SCENE`，不得在全部场景之后另设 UI 或弹窗尾部阶段。`gameplay` 与 `supporting` 只用于分类，场景之间不得绕过公开契约直接访问其他场景状态。
6. V5 在正式功能代码实现后执行运行态视觉接入、功能/视觉联合验收、响应式/性能与完整交互轨迹复验；V5 不是前置视觉方向审批。全部授权场景（含其显示层）闭环后，才实施跨场景导航、存档、音频、状态连续性、异常恢复等 `INTEGRATION`，并完成联合验收。
7. A4 才负责正式入口接入、迁移和高影响集成。只有 V2、V3、V4、正式功能实现、V5 和跨场景联合验收均关闭后，才允许退出 G1，进入 G2 完整集成与回归。首个可玩切片只是中间里程碑，不是 G1 出口。

## 单场景完成闭环

每个场景只有一个宿主 `SCENE` Work Item 闭环：V1 冻结功能/视觉合同与参考，V2 在同一 Work Item 内完成完整候选、动态样片、F2 机器 PASS 和唯一真人审批，V3 拆解，V4 完成正式资源和组合预验收，之后实现正式功能，V5 做运行态接入与功能/视觉联合复验，A4 才接正式入口。`DISPLAY_LAYER` 只是宿主场景的紧邻子单元，不是独立实现任务；它必须复核宿主上下文图与 scene/layer/host 身份。全局 `visual_baseline` 只负责静态风格，不能替代场景 V2。灰盒只用于临时证明结构、交互或节奏；仍引用灰盒、占位纹理或未验收资源的场景不得标记完成。
...
2. 适用 V1 时定义状态、输入、规则、反馈和必要低保真草图/灰盒；正式功能代码不得在 V1/V2 前写入。指定效果图还原时冻结参考身份、目标视口/状态、对比条件和可观察视觉事实；静态图无法证明的玩法、交互或动画标为待定义。只有不改变冻结视觉事实且处于项目预定义容差内的适配可 `AUTO`，可见偏差或实质取舍请求一次精确确认并绑定已批准例外。
3. V2 把完整场景候选装入当前 Work Item 的隔离视觉样片并提供动态轨迹；结构化 F2 机器检查覆盖识别、预警、反馈、遮挡和小屏表现，随后只进行一次真人视觉方向审批。V2 `COMPLETE/frozen` 是正式 A3 包和正式功能代码的唯一前置视觉边界；V2 前不得实现或注册 SHARED/MODULE/SCENE/DISPLAY_LAYER 正式单元。严格复刻可免探索，但不得免完整候选、动态样片、机器 PASS、唯一真人审批、同条件证据和逐状态/逐区域矩阵。
...
6. V4 生产并验证资源与正式组合；只有机器清单状态为 `accepted` 且来源或生成记录、授权、正式布局、运行时输出、Phaser 和玩法视觉证据完整时，才可交给后续正式功能实现。效果图区域还必须逐 `annotation_number/region_id` 显式声明七个生产合同字段以及状态/部件映射；不得在 V4 之前以占位资源启动正式功能代码。
...
7. V1/V2 视觉样片与 V5 正式结构沿用同一生产 Scene 入口/骨架逐步重构，但样片不得注册正式入口或承载正式业务逻辑。V4 之后实现正式功能，V5 由玩法和美术协作完成结构化接入、动态验收和低保真清理；正式资源绑定场景/覆盖区域，效果图模式以不可变 fidelity case 绑定双方 SHA 与同条件证据。
...
V2 `COMPLETE/frozen` 且 V4 正式资源与同屏组合预验收通过后，正式功能代码才进入 `executionUnits`；`SHARED` 骨架、共享契约/事件/状态接口和基础设施以及 `INTEGRATION` 单元强制串行，其间 `MODULE` 单元可按互斥所有权并行。模块完成后，按 `executionUnits` 数组逐场景推进，`DISPLAY_LAYER` 必须绑定 `displayLayerId` 与 `hostSceneId`，在数组中紧邻其宿主 `SCENE`，并在宿主场景装配门内完成。每个场景的玩法、正式资源和全部显示层必须一起验收；同一非空并行组必须连续出现，至少包含两个单元，且文件写范围、玩法/场景/显示层状态所有权不得相交。控制面只校验并执行该顺序，不计算依赖闭包或拓扑排序；READY 只检查目标位置之前的有效 PASS（并行组从组首项计算），同组 peer 不互相等待。跨模块或跨场景装配、共享入口替换与最终集成保持串行。串行 A3 用 `delegate-check`；并行 A3 必须把完整同组委派组成不可变批次并用 `parallel-check` 原子校验。
...
场景与显示层实现消费同一场景 Work Item 的视觉真值。V1 只冻结功能/视觉合同与参考，V2 必须先完成当前场景的完整候选、动态样片、F2 `MACHINE/PASS`、唯一真人审批和 scene master/宿主上下文图；V2 前仅允许隔离灰盒或无正式业务逻辑样片。V3 冻结实施拆解，V4 验收正式资源与宿主组合，之后才开始正式功能代码；scene master/reference target、modal/popup/drawer/toast 上下文图和原子资产的 generated 记录都必须绑定当前场景身份与全部锚点。provided 文件只记录来源。全局基线只负责静态视觉一致性，不能替代逐场景 V2；任何基线、锚点、目标或提示词身份变化都要使旧生成证据失效。
