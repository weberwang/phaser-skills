...
7. V5 由玩法和美术协作结构化装配：玩法维护规则、状态、碰撞和交互；美术调整纯表现和布局/表现数据，不得用整屏效果图代替独立 GameObject、容器、图层和布局结构。正式资源必须绑定当前 `scene_id`，或明确标记为符合公共提取条件的 `shared`；接入只改变表现。忠实还原在冻结目标视口/状态以同条件完整 viewport 更新忠实度矩阵，其他视口验证布局关系不变量；动态可玩轨迹与低保真清理进入 F3 工程证据。
