...
5. 普通资产在 schema 1.4 明确 `not-applicable`。效果图冻结后、进入 V3 前完成合同回对与 coverage，标记 `v3-ready`；先依据 ownership/F2 事实为每个区域登记稳定 `annotation_number` 和 `implementation_plan`，再直接在冻结效果图上生成内嵌原图的编号 annotated SVG，展示本次生成、复用既有资源和程序实现。只有固定区域的 `bitmap-decomposition + generate-now` 先提交绑定冻结目标 SHA/region ID/区域定义 SHA 的提案，并在 confirmation 记录 proposal/decision 文件及 SHA、编号 SVG 文件/版本/SHA、decision_id 和实际用户消息身份后等待一次 USER_DECISION；`reuse-existing` 必须通过不可变 `asset-reuse-snapshot/1.0` 文件检查，精确绑定 `source_file`、`source_manifest_sha256`、`source_sha256`、`compatibility_evidence_sha256`，冻结原图必须是与目标画布同尺寸的完整合法 PNG。复用和程序实现区域不触发位图确认。开始任何拆解生产前必须运行带 `--check-files --project-root .` 的校验且结构和文件证据均通过，确认前不得裁切、抠图、分层、AI 分割/补全或生产派生位图；V3/V4 可暂无 fidelity，V5 验证完成才改为 `v5-complete` 且全部 case 通过。验证器不从像素臆测 owner_type。
