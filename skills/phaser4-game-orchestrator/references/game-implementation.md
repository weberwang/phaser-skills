...
在任务授权、适用的 A0-A6/F0-F4 与 V0-V5 硬门允许的边界内，执行/实施使用最小充分事实而不是穷尽式研究：入口、关键调用链或契约、范围、主要风险和验收目标明确且无直接冲突后，立即停止探索；A1/A2 冻结当前候选的范围、假设与验收边界，直接进入适用执行/验证；A3 冻结 `Implementation Package` 后进入实施。可逆、本地的 A1/A2 可记录合理假设后落地，A3 可记录合理假设后实施但必须先冻结包；缺少完全证明不单独阻塞。
...
2. 场景 V1 时定义状态、输入、规则、反馈和必要低保真草图/灰盒；正式场景功能代码不得在 V1/V2 前写入。指定效果图还原时冻结参考身份、目标视口/状态、对比条件和可观察视觉事实；静态图无法证明的玩法、交互或动画标为待定义。只有不改变冻结视觉事实且处于项目预定义容差内的适配可 `AUTO`，可见偏差或实质取舍请求一次精确确认并绑定已批准例外。
3. 场景 V2 把完整候选装入当前 Work Item 的隔离视觉样片并提供动态轨迹；结构化 F2 机器检查覆盖识别、预警、反馈、遮挡和小屏表现，随后只进行一次真人视觉方向审批。V2 `COMPLETE/frozen` 是包含场景/集成单元的正式 A3 包和场景功能代码的唯一前置视觉边界；基础阶段的 foundation-only 包不属于场景 V2，但必须先有全局静态基线冻结。严格复刻可免探索，但不得免完整候选、动态样片、机器 PASS、唯一真人审批、同条件证据和逐状态/逐区域矩阵。
...
场景与显示层实现消费同一场景 Work Item 的视觉真值。基础实施完成后、场景 V1/V2 开始前，先冻结 scene master、宿主上下文图和对应视觉合同；V1 冻结功能/视觉合同与参考，V2 随后完成当前场景的完整候选、动态样片、F2 `MACHINE/PASS` 和唯一真人审批。场景 V2 前仅允许隔离灰盒或无正式业务逻辑样片。V3 冻结实施拆解，V4 验收正式资源与宿主组合，之后才开始正式场景功能代码；scene master/reference target、modal/popup/drawer/toast 上下文图和原子资产的 generated 记录都必须绑定当前场景身份与全部锚点。provided 文件只记录来源。全局基线只负责静态视觉一致性，不能替代逐场景 V2；任何基线、锚点、目标或提示词身份变化都要使旧生成证据失效。
