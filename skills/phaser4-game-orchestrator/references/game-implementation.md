...
2. 在任何骨架代码或 A3 代码单元前，先完成并冻结全局视觉效果图集合：覆盖全部授权 `gameplay`/`supporting` 场景的 `scene master`，以及所有必需瞬态显示层状态的宿主场景上下文效果图。集合按 scene/state 分项冻结，不是一张合并图；冻结门保留 V0-V2 方向与唯一真人视觉审批，并登记完整 coverage/layout/fidelity obligations。它是进入实施的前置门，不是新增 `executionUnit` 类型，也不宣称 V3-V5 已完成；effect-image 仍完整执行 V1→V5，V3-V5 资源生产与运行接入在对应场景阶段完成。
3. 通过全局视觉冻结门后，先实施 `SHARED` 最小运行骨架：Boot/Preload、Scene 生命周期、模块公开接口、输入、资源注册、统一布局入口、平台适配和测试支撑。只提供稳定契约和最小运行能力，不实现完整模块或场景业务；公共代码或正式资源仅在至少两个已确认场景稳定复用，或属于运行必需时提取，禁止无边界 `common`、`utils` 和公共素材库。
4. 骨架稳定后实施 `MODULE`。模块实现位于骨架之后、场景之前；可按互斥文件写范围和状态所有权安全并行，共享契约、公共入口和基础设施保持串行。模块不得提前占有场景业务或替代场景闭环。
5. 模块完成后，按计划制定者冻结的实际场景顺序逐个实施 `SCENE`。每个场景必须把玩法、正式资源、全部 HUD/UI/modal/popup/drawer/toast 等显示层、生命周期清理和功能/视觉/响应式/性能联合证据一次闭环；其 `DISPLAY_LAYER` 子单元必须紧邻并归属于宿主 `SCENE`，不得在全部场景之后另设 UI 或弹窗尾部阶段。`gameplay` 与 `supporting` 只用于分类，场景之间不得绕过公开契约直接访问其他场景状态。
6. 全部授权场景（含其显示层）闭环后，才实施跨场景导航、存档、音频、状态连续性、异常恢复等 `INTEGRATION`，并完成联合验收；确认所有授权功能均有归属场景或跨场景所有者。
7. 只有全局视觉冻结、全部授权场景/功能、正式视觉和跨场景联合验收均关闭后才允许退出 G1，进入 G2 完整集成与回归。首个可玩切片只是 G1 中间里程碑，不是 G1 出口。

## 单场景完成闭环

每个场景以宿主 `SCENE` 闭环完成：玩法功能与测试、V3 正式资源规划、V4 `accepted`、V5 结构化正式接入、全部 UI/显示层状态、灰盒/占位/fallback 清理，以及功能、视觉、响应式和性能联合证据。全局视觉冻结阶段已盘点该场景的 HUD、modal、popup、drawer、toast 等显示层；scene master 只冻结基础场景和常驻层，瞬态层分别提供带宿主场景、遮罩/层级和当前状态的上下文效果图。孤立透明组件图不能作为完整效果图或最终验收证据。`DISPLAY_LAYER` 只作为宿主场景的紧邻子单元执行，不得形成独立尾部阶段。灰盒只用于临时证明结构、交互或节奏；仍引用灰盒、占位纹理或未验收资源的场景不得标记完成。
...
3. 全局视觉效果图冻结门通过后，按 `executionUnits` 数组实施 `SHARED`→`MODULE`→按场景组织的 `SCENE`+紧邻 `DISPLAY_LAYER`→`INTEGRATION`。模块单元可按互斥所有权并行；共享契约/入口、场景装配和跨场景联合验收保持串行。数组由计划制定者冻结为权威顺序，READY 只按数组位置和当前候选完成证据校验而非手工填写或依赖推导；不得把纯玩法规则或 UI/弹窗单独移到其宿主阶段之外，也不得消费未验收资源或从效果图裁切正式资源。
...
场景与显示层实现消费同一全局视觉真值。A3 实施包/代码单元前，必须先冻结覆盖全部授权场景的 scene master 与必需瞬态宿主上下文效果图，并确认 `visual_baseline` 已冻结、`anchor_evidence` 完整、V2 唯一真人审批和适用 coverage/layout/fidelity 硬门通过；scene master/reference target、modal/popup/drawer/toast 上下文图和原子资产的 generated 记录都必须绑定该身份与全部锚点。provided 文件只记录来源。全局基线只负责静态视觉一致性，不能替代 V2 方向冻结；任何基线、锚点、目标或提示词身份变化都要使旧生成证据失效。
