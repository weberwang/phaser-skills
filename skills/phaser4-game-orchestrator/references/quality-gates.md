...
首个可玩切片是 G1 中间里程碑，只能证明核心链路可运行。基础包完成不代表场景 V2/V3/V4 完成，也不允许提前接入正式可见 Scene。存在未完成授权场景/功能、未达到 V3 `accepted` 或未完成 V4 的正式资源、占位/fallback 引用、缺失响应式或性能证据时，不得宣称 G1 完成。G2 不补做遗漏场景、功能或正式视觉，而是验证 G1 已关闭的完整候选。
