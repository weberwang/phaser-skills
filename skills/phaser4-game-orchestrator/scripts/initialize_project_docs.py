...
| 资源 | 单图 ID/来源版本 | 生成状态 | 类型 | 用途 | 来源 | 授权/生成记录 | 规格/锚点 | 交付路径/纹理键 | Phaser 验证 | 发布可用 | 优化状态 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |

## 美术生产与单图验收

| 目标预制体/场景 | 低保真路径与版本 | A1 确认 | 高保真效果图与版本 | A2 确认 | A3 审阅 | A4 框选图/单图清单 | A4 确认 | A5 单图验收 | 正式装配证据 | 低保真清理证据 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |

A4 确认后，优先按来源内容哈希与框选编号生成唯一拆分 ID；无法计算哈希时才使用规范化来源路径、明确版本与框选编号。查询上表并复用文件存在、纹理键可寻址且验证仍有效的正式产物。逐项记录用途、边界、规格、锚点、纹理键、生成或补绘方式及复用/新生成/重生成结论。框选图、清单、来源版本或规格变化后必须返回 A4 重新确认。

单图生成后逐项记录透明边缘与完整性检查、深浅底对比预览、实际加载方式、目标缩放显示结果及复现证据，并将状态更新为“已生成”。旧版本标为“已取代”，不得静默覆盖。正式结构验收后删除低保真实现并记录删除后的测试和构建证据；任一确认门、正式装配或低保真清理未通过时不得标为可交付。
