...
description: "Phaser 4 + Capacitor 移动项目接入 AppLovin MAX 聚合广告时使用；指导 iOS/Android 原生桥接、Banner、插页与激励视频、多广告位静默预加载、统一失败重试、视频不可用提示、插页广告位独立 60 秒冷却与隐私验收，不用于 Phaser Web 或小游戏广告实现。"
---
...
- Banner 初始化后创建为隐藏状态并静默加载，只有已 ready 且布局安全时才能立即显示；未 ready 时保持隐藏并立即返回，不弹 Toast、不占用错误高度、不等待加载。Banner 不使用视频 Toast、全屏仲裁或插页广告位局部冷却。
...
3. 冻结桥接契约：`initialize`、`preload(slotId)`、`tryShow(context)`、`setBannerVisibility(slotId, visible)`、`isReady(slotId)` 均为非阻塞调用并返回结构化结果；初始化失败进入 `failed`，瞬时失败只能通过显式初始化重试入口恢复，且不能与加载重试混用。服务初始化幂等，每个广告位只有一个状态所有者和一个格式匹配的 MAX 对象。初始化成功后立即静默预加载全部启用广告位；所有加载失败统一交给一个服务级重试协调器，按广告位保存计数与 deadline，并应用同一套 `2/4/8/16/32/64 秒`退避、抖动、去重、后台/离线暂停和恢复规则。
...
5. 在自然中断点调用 `tryShow`；调用只读取指定广告位快照，绝不触发前台加载。不得建立跨广告位的全局冷却；每个插页广告位独立维护 60 秒局部冷却。只有收到原生 MAX 已确认插页展示成功且与当前 `instanceGeneration`/`slotId`/`showRequestId` 匹配的回调后，才用单调时钟更新该广告位的冷却；一个插页广告位进入冷却不得阻断其他插页广告位。未 ready、当前广告位冷却中、正在展示、隐私未决、网络不可用或平台不支持都立即返回并继续游戏；视频广告不可用或展示失败时，由统一 UI 层消费结构化结果/事件并显示一次不可用 Toast。激励只由匹配的 `rewarded` 回调异步发放且保持幂等；隐藏或展示失败后继续静默预加载。
...
结果至少包含稳定的 `ok`、`code`、`platform`、`phase`、`gates` 和必要的 `retryScheduled`/`cooldownRemainingMs` 字段；`cooldownRemainingMs` 只描述本次请求所指插页广告位的局部冷却。Web 层配置只传环境和能力开关，永不传 SDK key、MAX ad unit ID 或网络凭证。不得把 SDK 原始异常、设备 ID 或完整 waterfall 凭证透传到 Web 层。`tryShow` 返回“已请求展示”不等于已展示成功；局部冷却只由与当前代次、广告位和请求 ID 匹配的原生 `displayed` 事件驱动。
...
- 生命周期：初始化幂等；每个已配置广告位持有对应格式的 MAX 对象、phase、加载计数和 retry deadline，每个插页广告位另持有自己的冷却时间戳；所有广告位共享一个重试协调器与全屏展示仲裁，但不共享冷却；实例重建递增 `instanceGeneration`，展示请求生成唯一 `showRequestId`；成功初始化立刻静默预加载全部启用广告位，重复调用不会并发请求。
...
- 节奏：自然中断点展示；插页展示成功后只让匹配的插页广告位进入 60 秒局部冷却，其他插页广告位不受影响；不存在跨广告位或跨格式的全局冷却。失败、未 ready 和被跳过不消耗冷却；`hidden` 与 `displayFailed` 都会触发后台预加载。
- 失败恢复：所有广告位使用同一个加载失败重试协调器和同一套错误分类/退避公式；计数、deadline 按广告位隔离，协调器统一去重和调度，后台/离线统一暂停，恢复后每个到期广告位最多恢复一次；展示失败独立记录并重新预加载，不进入另一套重试逻辑。
