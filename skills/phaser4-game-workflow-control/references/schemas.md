...
V4 的合同还必须声明 `combination_preacceptance`，固定视觉实施单元必须提供完整 `scene_asset_usage`；F2 `visual_fidelity_review` 与 `production_contract_review` 都必须带目标/候选身份、整屏比较、逐区域结构化审查和机器/AI evidence，但不要求第二 reviewer 或重复 `human_review`。旧的独立资源或缺任一新增 effect-image 字段的工件不兼容当前 schema。
