...
- 程序化文本纳入实施与审查硬约束：必须同时实现英语 `en`、简体中文 `zh-CN`、日语 `ja`、俄语 `ru`、西班牙语 `es`，并以各语言真实字形宽度、基线、字体回退和容器适配完成显示验证。文案在语义准确、玩家可理解的前提下保持短促、游戏化，优先使用 `x3`、`+10`、图标+数值等熟悉表达，不写可由符号直接表达的说明句。可见程序化文本默认保持单行，禁止截断和自动换行；放不下时应按语言改短文案、调整字距/字号或扩展容器，同时保持可读字号。
