...
effect-image ImageGen 的 canonical 提示词、真实参考输入和 generation_record 绑定规则统一见 [`effect-image-prompt-contract.md`](../phaser4-game-asset-integration/references/effect-image-prompt-contract.md)；控制面这里只校验路由、硬字段和退回阶段，不重复模板正文。
