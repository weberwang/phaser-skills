...
description: Phaser 4 游戏仓库的唯一全局工作流控制面。基于任务授权和确定性风险分类放行低风险本地工作，并对实质取舍、外部写入、破坏性、真机与发布保留精确显式批准。
---
...
- 仅 A4、A5、A6 或实质用户取舍创建 pending。短回复只解释为最近 `handoff` 展示的唯一 pending，不能批准后续门或扩展范围。
- A4 默认需要 F4 精确批准；A5 必须绑定一次性外部目标批准；A6 包括破坏性、生产迁移、真机、商店/正式发布和线上回滚，永不自动。
