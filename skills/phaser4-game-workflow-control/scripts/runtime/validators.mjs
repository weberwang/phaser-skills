...
export function createRecordValidators({
  approvalFields, delegationRequired, delegationFields, evidenceRequired, packageRequired, changeRequired,
  ...
  function validateApproval(approval) {
    requireFields(approval, approvalFields, `审批 ${approval?.approvalId ?? '<unknown>'}`);
  ...
  function validateDelegation(delegation) {
    requireFields(delegation, delegationRequired, 'Delegation Package');
    const extra = Object.keys(delegation).filter((field) => !delegationFields.includes(field));
    ...
    if (!actionLevels.includes(delegation.actionLevel) || !['A0', 'A1', 'A2', 'A3'].includes(delegation.actionLevel)) fail('Delegation Package 只能委派 A0-A3 Phaser 动作');
  ...
  function validateEvidence(evidence) {
    requireFields(evidence, evidenceRequired, 'Evidence Manifest');
  ...
  function validateImplementationPackageShape(pkg, options = {}) {
    requireFields(pkg, packageRequired, 'Implementation Package');
    const extra = Object.keys(pkg).filter((field) => !packageRequired.includes(field) && !['visualProductionUnits', 'visualManifestFile', 'visualManifestSha256', 'visualContractVersion', 'candidateVersion', 'visualDecompositionConfirmations', 'current_stage', 'currentStage', 'sceneReconstructionContract', 'scene_reconstruction_contract'].includes(field));
  ...
  function validateChangeRequestShape(change) {
    requireFields(change, changeRequired, 'Change Request');
