...
export function createStableCommands(deps) {
  const inspect = (args, command) => inspectWorkflow(args, command, deps);
  ...
  function run(args) {
    ...
    deps.transition({ ...before.args, to: target, silent: true, object: before.work.pendingApprovalObject, 'action-type': before.work.pendingApprovalActionType, 'external-target': before.work.pendingApprovalExternalTargets });
    const after = inspect(args, 'run');
...
function inspectWorkflow(rawArgs, command, deps) {
  ...
  if (packagePath) {
    ...
    try {
      packageValue = deps.readJson(packagePath, 'Implementation Package');
      implementationPackage = deps.validateImplementationPackage(packageValue, work, repo);
    } catch (error) {
      packageError = error;
  ...
  if (!packageError && implementationPackage) {
    const snapshot = deps.loadVisualManifestSnapshot(implementationPackage, repo);
  ...
  if (work.globalState !== 'RETURN') {
    try {
      visualResult = deps.validateVisualStagePrerequisites({ ...work, implementationPackage, visualManifest }, {
        ...
        visualManifest,
        evidence: args.evidence ? deps.readJson(args.evidence, 'Evidence Manifest') : null,
      });
  ...
  if (args.evidence && !packageError) {
    try {
      evidence = deps.evidenceCheck({ ...args, silent: true }, true);
    } catch (error) {
  ...
  let route = null;
  try {
    const ledger = args.ledger ? deps.readLedger(args.ledger) : { schemaVersion: '1.0', approvals: [] };
  ...
  return { args, command, repo, workPath, work, packagePath, packageValue, implementationPackage, executionState, visualResult, evidence, route, blockers, planFingerprint };
}
...
function nextAction(inspection) {
  ...
  if (work.globalState === 'IMPLEMENTING' && work.pendingApprovalActionLevel === 'A3' && executionState?.workflowState !== 'COMPLETE') return '完成当前 Execution State 的 READY 实施单元';
