...
test("确认前 v2_structured_review 只允许机器事实", () => {
  assert.deepEqual(validateVisualPostApprovalReviewFields({ v2_structured_review: machineReview(), visual_human_approval: approval() }, { stage: "V2" }), []);
...
test("人工确认通过后没有重复复核工件即可通过机器字段门", () => {
  const manifest = { visual_human_approval: approval(), candidate_identity: { sha256: SHA, diff_fingerprint: "diff-1" }, fidelity_cases: [{ candidate_identity: { sha256: SHA, diff_fingerprint: "diff-1" } }] };
  assert.deepEqual(validateVisualPostApprovalReviewFields(manifest, { stage: "V5" }), []);
});
...
test("post-approval 重复复核工件和 reviewer 字段明确拒绝", () => {
  for (const key of ["f2_review", "f2_reviews", "visual_fidelity_review", "production_contract_review", "component_reviews", "reviewer", "reviewer_type", "reviewer_id", "review_id", "reviewed_at", "human_review"]) {
    const errors = validateVisualPostApprovalReviewFields({ [key]: key === "reviewer" ? "qa" : {} }, { stage: "V5" });
    assert(errors.some((value) => value.includes(key)), `${key} 应被拒绝`);
  }
});

test("缺失或身份漂移的唯一 V2 审批仍失败", () => {
...
test("V3-V5 候选 SHA 正常演进不触发第二次人工确认", () => {
  const manifest = { visual_human_approval: approval(), candidate_identity: { sha256: `sha256:${"c".repeat(64)}`, diff_fingerprint: "runtime-diff-v5" } };
  assert.deepEqual(validateVisualPostApprovalReviewFields(manifest, { stage: "V5" }), []);
});
