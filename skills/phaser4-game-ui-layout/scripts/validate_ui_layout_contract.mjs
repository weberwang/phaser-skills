...
const ROOT_REQUIRED = ["schema_version", "contract_id", "contract_version", "scope", "targets", "coordinate_spaces", "regions", "content", "platform_insets", "scrolling", "dynamic_content", "overlay_rules", "breakpoints", "invariants", "evidence_matrix"];
...
function validateRoot(document, errors) {
  ...
  for (const field of ["coordinate_spaces", "regions", "overlay_rules", "breakpoints", "invariants"]) if (field in document && !Array.isArray(document[field])) errors.push(`字段 ${field} 必须是数组`);
  for (const field of ["scope", "targets", "content", "platform_insets", "scrolling", "dynamic_content", "evidence_matrix"]) if (field in document && !isObject(document[field])) errors.push(`字段 ${field} 必须是对象`);
}
...
function validateBreakpoints(breakpoints, errors) {
  if (!Array.isArray(breakpoints) || breakpoints.length === 0) { errors.push("breakpoints 必须是非空数组"); return; } const seen = new Set();
...
function validateDynamicContent(dynamic, ids, errors) {
  ...
  if (!Array.isArray(dynamic.key_actions) || dynamic.key_actions.length === 0) errors.push("dynamic_content.key_actions 必须是非空数组"); else dynamic.key_actions.forEach((action, index) => { if (!isObject(action)) { errors.push(`dynamic_content.key_actions[${index}] 必须是对象`); return; } if (!isString(action.id)) errors.push(`dynamic_content.key_actions[${index}].id 必须是非空字符串`); else if (!ids.has(action.id)) errors.push(`dynamic_content.key_actions[${index}].id 引用不存在的区域：${action.id}`); const required = new Set(["default", "disabled", "submitting", "completed"]); if (!Array.isArray(action.states) || action.states.length === 0 || !action.states.every(isString)) errors.push(`dynamic_content.key_actions[${index}].states 必须是非空字符串数组`); else { const missing = [...required].filter((state) => !action.states.includes(state)).sort(); if (missing.length) errors.push(`dynamic_content.key_actions[${index}].states 缺少必需状态：${missing.join(", ")}`); } if (!["forbid", "forbid-critical"].includes(action.text_truncation)) errors.push(`关键动作禁止文本截断：dynamic_content.key_actions[${index}]`); });
...
export async function main(argv = process.argv.slice(2)) {
  const asJson = argv.includes("--json"); const positional = argv.filter((item) => item !== "--json"); let result;
  try { if (positional.length !== 1) throw new Error("必须传入一个合同文件路径"); result = validateContract(await loadContract(positional[0])); } catch (error) { result = { status: "failed", errors: [`无法解析合同：${error.message}`], warnings: [], specialized_review: [] }; }
