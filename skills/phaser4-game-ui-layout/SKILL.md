...
程序化文本必须同时交付英语、简体中文、日语、俄语和西班牙语，并逐语言验证字形宽度、字体回退、基线与容器适配。可见文案保持单行且不得截断或自动换行；空间不足时优先压缩文案、调整字距/字号或扩展容器，但不得降到不可读字号。表达应短促、游戏化，倍率统一写成 `x3` 这类 `x{value}` 形式，数值变化优先使用 `+10`、图标+数值等熟悉表达，避免解释性长句。
