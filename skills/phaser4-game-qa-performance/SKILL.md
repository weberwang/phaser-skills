...
可复用自动化脚本位于 `scripts/responsive-visual-validation.mjs`，纯计算测试位于 `scripts/responsive-visual-validation.test.mjs`。脚本通过 Node ESM 动态导入 Playwright；工作流只使用 DPR 2，在同一页面通过 `setViewportSize` 调整视口并保留 resize 语义；声明 DPR 1、3 或字符串 `"2"` 的配置在验证前失败。报告输出完整页面截图、运行时实测矩阵和结构化 JSON。
