...
8. UI 读取 [`phaser4-game-ui-layout`](../phaser4-game-ui-layout/SKILL.md) 合同，并按 [响应式视觉验证](references/responsive-visual-validation.md) 采集真实稳定帧。默认 `usability` 至少覆盖窄/标准竖屏、横屏、桌面宽屏、DPR 1/1.25 或 1.5/2/>2 封顶、同页连续 resize、降至 DPR 1、DPR 不变 resize 和代表性弹窗轨迹；只有 `exact` 或项目明确要求时才扩展到完整断点和状态矩阵。每个视口直接记录 CSS 矩形、Canvas backing、raw/effective DPR、逻辑↔CSS↔物理比例、Camera、输入命中、安全区、关键 UI、截图及 resize 轨迹，不能用推导值冒充实测。正式报告绑定候选 SHA、scene/state、布局合同版本和视觉基线版本；缺测量为 `unverified`，宿主场景证据不能代替独立 DISPLAY_LAYER。
...
可复用自动化脚本位于 `scripts/responsive-visual-validation.mjs`，纯计算测试位于 `scripts/responsive-visual-validation.test.mjs`。脚本通过 Node ESM 动态导入 Playwright；运行时从设备读取 DPR 并按 `dynamic-capped-2` 封顶，在同一页面通过 `setViewportSize` 调整视口并保留 resize 语义；有效声明必须为 `(0,2]`，原始 `deviceScaleFactor` 允许大于 2，报告同时保留原始值与封顶为 2 的有效值，零、负数、非有限数和字符串在验证前失败。默认报告输出代表性视口与关键状态；`exact` 模式或明确的全覆盖需求再输出完整页面截图、实测矩阵和结构化 JSON。
