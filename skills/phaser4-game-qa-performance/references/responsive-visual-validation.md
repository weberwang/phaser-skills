...
- **V2 F1**：同一页面按矩阵运行动态样片，覆盖重锚定、间距、背景覆盖、安全区、
  层级、最小可读性、显隐、滚动、触控和 resize；只有基准静态图阻断 V3。
- **V4 F2**：非作者分别检查资源的满幅能力、锚点/字体/九宫格/缩放和预算；不能
  用算法验证或 Canvas ROI 代替生产场景证据。
- **V5/F3**：非作者在运行态覆盖所有矩阵和关键动态状态，复核实际 viewport 覆盖、
  safe area、UI 边界、动态 resize 和性能峰值。缺完整 viewport 截图、轨迹或 Hook
  结论只标记 `unverified`，不得推断通过。
