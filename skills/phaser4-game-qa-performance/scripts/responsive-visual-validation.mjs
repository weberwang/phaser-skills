...
export function readHookData(snapshot) {
  ...
  const logicalSize = logical && finiteNumber(logical.width) !== null && finiteNumber(logical.height) !== null
    ...
    : null;
  const safeArea = snapshot.safeArea && typeof snapshot.safeArea === "object"
    ? {
        ...
        rect: normalizeRect(snapshot.safeArea.rect)
      }
    : null;
  return {
    present: true,
...
export function buildResizeRecords(measurements, contract = {}) {
  ...
  for (let index = 1; index < measurements.length; index += 1) {
    ...
    records.push({
      ...
      pageReloaded: Boolean(before.pageReloaded || after.pageReloaded),
  ...
  const trajectoryStatus = records.some((record) => record.pageReloaded)
    ? "fail"
...
export function evaluateMatrixCoverage(measurements, contract = {}) {
  ...
  const status = expected.length === 0 || missing.length > 0 ? "decision_gap" : "pass";
  return { expected, observed: [...observed], missing, status };
}

/** 汇总视口、Hook 和 resize 结果，供 CLI 与无浏览器测试共用。 */
export function summarizeReport(measurements, contract = {}) {
  const normalized = normalizeContract(contract);
  ...
  const status = worstStatus([...viewportStatuses, resize.status, matrix.status]);
  return {
    status,
    responsivePass: status === "pass",
...
async function runBrowserValidation(options) {
  ...
  const contract = normalizeContract(await readJsonInput(options.contract));
  ...
  const dpr = finiteNumber(first.deviceScaleFactor) ?? 1;
  const browser = await chromium.launch({ headless: true });
  const context = await browser.newContext({ viewport: { width: first.width, height: first.height }, deviceScaleFactor: dpr, isMobile: Boolean(first.isMobile), hasTouch: Boolean(first.hasTouch) });
  ...
  try {
    ...
    for (let index = 0; index < viewports.length; index += 1) {
      ...
      const pageReloaded = index > 0 && navigationCount > navigationBefore;
      ...
      measurements.push({ name: viewport.name, declaredViewport: viewport, pageReloaded, navigationCount, ...evaluated });
    }
  } finally {
    await context.close();
    await browser.close();
  }
  const summary = summarizeReport(measurements, contract);
  const report = { generatedAt: new Date().toISOString(), url: String(options.url), contract, ...summary };
...
/** 输出最小 CLI 帮助文本。 */
function printHelp() {
  console.log("用法：node responsive-visual-validation.mjs --url URL --viewports MATRIX --canvas-selector canvas --contract CONTRACT --output DIR [--hook NAME]");
}

/** CLI 入口；纯计算函数可在无浏览器测试中直接导入。 */
export async function main(argv = process.argv.slice(2)) {
  const options = parseArguments(argv);
  if (options.help || !options.url || !options.viewports || !options.contract) {
    printHelp();
