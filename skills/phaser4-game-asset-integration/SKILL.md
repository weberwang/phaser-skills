...
同一场景 Work Item 的 V0→V4 链在 V2 依次完成拆解确认和布局确认，冻结还原方案。`visual-decomposition-confirmation/1.0` 必须绑定 annotation/proposal/decision SHA、target、V2 candidate、diff、baseline、全部编号和用户原文；任一绑定漂移后确认失效并重新回到当前 Work Item 的 V2。V2 拆解图、技术 JSON、合同回对、coverage 和结构化机器检查必须齐全，布局确认另行绑定最终布局图与决策文件。V3 actual asset/component×state/同屏组合以及 V4 full viewport、overlay、diff、逐区域 fidelity、F2 视觉一致性与生产合同检查全部使用绑定当前身份的确定性机器证据。根节点 PASS、布尔值、AI reviewer 字段或旧 `human_review` 均不能代替两次 V2 确认。
