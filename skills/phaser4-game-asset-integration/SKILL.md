...
4. schema 1.4 `visual-assets.json` 先声明 `effect_image_reconstruction`：普通资产为 `not-applicable`，不要求还原工件；效果图还原为 `effect-image`。后者冻结目标后、进入 V3 前以 `v3-ready` 完成合同回对和 coverage，V3/V4 可暂无 fidelity case；只有 V5 验证完成才为 `v5-complete` 并要求全部 case 通过。效果图 coverage 逐区登记 `annotation_number` 与 `implementation_plan`，先完成 ownership/实现分类，再在冻结效果图上生成框选 annotated SVG 并展示给用户：`generate-now`（本次生成）、`reuse-existing`（复用既有资源）、`runtime-program`（程序实现）。固定区域必须区分 `bitmap-decomposition` 与 `independent-production`；只有 bitmap 的 `generate-now` 区域等待一次精确 `USER_DECISION`，确认中写入 `proposal_id`、目标 SHA、`region_id`、区域定义 SHA、提案/决定记录及 SHA、编号 SVG 文件/版本/SHA、`decision_id` 和实际消息身份字段。`reuse-existing` 必须使用不可变 `asset-reuse-snapshot/1.0`，并绑定 `source_file`、`source_manifest_sha256`、`source_sha256`、`compatibility_evidence_sha256`，文件检查确认快照资源为 `accepted`；冻结原图必须是与目标画布同尺寸的完整合法 PNG。确认后、任何裁切/抠图/分层/AI 分割或补全前，必须运行带 `--check-files --project-root .` 的校验并通过；复用和程序实现区域不触发位图确认但必须在同一标注图中可见。V3 再按 [资产生产路线](references/asset-production-routes.md) 选择路线。
