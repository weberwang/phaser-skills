...
`effect-image` 的 AI 合成栅格路线必须遵循[Effect-image ImageGen 忠实还原提示词合同](effect-image-prompt-contract.md)；本文件只负责路线选择、交付形态与阶段分工。
...
当 `expected_assets.alpha=true` 时，ImageGen 首选直接生成真实透明背景并交付 PNG（`mime_type=image/png`、`.png`）；生成记录默认声明 `background_mode=transparent`、`transparency_strategy=direct-generation`，实际提示词必须包含透明直出要求。只有直接生成明确 `failed` 或 `unsupported` 时，才允许使用唯一兜底值 `transparency_strategy=background-removal-fallback`。兜底必须绑定结构化 `direct_generation_attempt`（`status` 只能为 `failed/unsupported`，并包含 `record_id`、`attempted_at`、`failure_reason`、`evidence`），并提供恰好一条 `background_removal_attempts`，其中明确写 `operation` 和完成状态；`operation`、`command_or_recipe` 或 `postprocess` 文本只用于确认实际背景移除，不用于跨字段计数。失败即退回 V3/V4，禁止无限重试或自动多次去背。直接策略仍禁止上述背景移除操作，且 `postprocess` 可以为空数组。无论哪条策略，V4 都须解码真实 PNG 并证明存在透明像素，不能只相信声明。

ImageGen 单图的固定生产顺序是“生成原图 →（透明直出失败/不支持时才做一次受控背景移除）→ 尺寸归一化 → V4/final/runtime”。原始生成文件永远是中间产物；归一化必须用项目根依赖 Sharp 生成最终 PNG/JPEG（`alpha=true` 只允许 PNG，`alpha=false` 可使用 JPEG），并写入 `normalization_record`。沿用既有字段职责：`expected_assets.source_file`、资产/生成记录的 `source_file` 指 ImageGen 原始中间图，`runtime_file`、`runtime_outputs` 和 `normalization_record.output_file` 指归一化交付物。源图与 `expected_assets.width/height` 宽高比必须通过交叉乘法一致，`padding_policy=none`，比例不一致必须重新生成，禁止 crop、padding、contain 或静默拉伸；原图尺寸已经正确时记录 `operation=not-required`。透明路线还必须在归一化前后确认 `hasAlpha=true`，归一化记录缺失、失败、路径/哈希/尺寸不一致均退回 V3/V4。
