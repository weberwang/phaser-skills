...
V3 为每个资源选择一条主路线，并在机器清单记录源文件、运行时输出、关键验收和预算。正式资源必须保留可编辑源文件；纯生成资产必须保留足以重现和审计的生成记录。
...
| AI 合成栅格拆分 `ai-composite-raster` | 分层重绘文件，或提示词、模型、参数、种子、输入与编辑记录 | 独立透明位图/图集及清单 | 框选编号、边缘补绘、透明度、尺度、锚点、可复现性与授权 | 生成批次、输出数量、纹理内存、图集、包体 |
...
记录 `schema_version`、全局预算、唯一 `id`、唯一 `texture_key`、唯一运行时输出路径、路线、状态、可编辑源文件或生成记录、授权记录、运行时输出、Phaser 证据和玩法视觉证据。已验收资源缺一项即不得标为 `accepted`。
