...
视觉来源与场景装配是两个独立决策。每个区域先根据冻结参考图的可观察事实选择 `image-asset`、`phaser-native` 或 `composite`，再用独立的 `assembly_analysis` 证明正式 Scene 采用原子装配且没有整屏捕获。`uses_full_screen_capture=false` 不得推导 `runtime-program`；原子 Sprite、NineSlice、独立背景块和其他图片部件仍是结构化实现。全部区域都选择 `phaser-native` 时，场景合同必须提供绑定冻结目标、完整 region 集合和独立 JSON 工件 SHA 的 `all_native_justification`；文件门会复算 SHA 并逐区核对资格事实，不能由同一拆解提案用“均为简单视觉”的自声明自行闭环。复合路线中的 `appearance` 必须使用 `image-asset`，`behavior` 才使用 `phaser-native`。
