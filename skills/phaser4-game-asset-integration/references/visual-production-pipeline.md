...
所有任务执行适用 V1/V2 专业检查。已有明确需求、参考或冻结基线且只忠实实现/专业修复时记录 `AUTO`；新方向、同等方案、可见结构/交互变化或高返工取舍只请求一次选择并记录 `USER_DECISION`。后续绑定当前决策记录。
...
V2a/V2b 和独立美术 F2 通过后，明确参考/基线或专业修复写入 `AUTO` 决策记录；存在实质视觉取舍时提出一次精确选择问题并记录 `USER_DECISION`。
...
V3 使用仍适用的 V1/V2 `AUTO` 或 `USER_DECISION` 记录、视觉可交付与全局基线。
...
适用 V 阶段证据、F0-F3、仅 A4-A6 的 F4、V1/V2 `AUTO` 或 `USER_DECISION` 记录、视觉/工程可交付、冻结基线、资源绑定、预算、授权和动态证据全部有效后才能报告完成。
