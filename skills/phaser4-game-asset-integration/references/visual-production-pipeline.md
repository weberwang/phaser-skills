...
2. 每项图片必须记录 CSS 逻辑显示范围、最大 intended scale、生产分辨率、source/runtime 文件尺寸，以及在代表性视口中是否会被放大；分辨率不足时阻断 V3，简单插值放大不得作为清晰度修复。生成式图片仍使用固定 1.5 倍生产基线，不能用运行时动态 DPR 或其上限 2 替代。
...
7. 每个生产包提交跨资源联系表与同屏组合截图，引用具体区域和可观察事实，由机器校验器检查角色、图标、面板、按钮、场景对象与 VFX 的形状、比例、材质、光源、描边、色彩和渲染密度。总控只核对基线绑定和证据完整性。
8. 忠实还原逐资源和同屏核对其是否支持冻结视觉事实；不得用整屏铺图、隐藏层或不可交互栅格绕过结构化实现及差异审计。
