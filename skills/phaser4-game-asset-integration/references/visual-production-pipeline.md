...
ImageGen 的源文件、运行时文件和实际输出仅允许 `image/png` 或 `image/jpeg` 以及 `.png`、`.jpg`、`.jpeg` 后缀；通用 authored-raster 可使用其他位图格式。`expected_assets.alpha=true` 的透明 ImageGen 单图是例外中的收紧分支：必须使用 `image/png` 与 `.png`，在生成记录中声明 `background_mode=transparent`、`transparency_strategy=direct-generation`，并在实际提示词中要求直接生成真实透明背景；不得使用抠图、去背、背景移除或 `matting/remove-background` 后处理，`postprocess` 可为空数组。V4 仍须解码 PNG 证明存在透明像素。
