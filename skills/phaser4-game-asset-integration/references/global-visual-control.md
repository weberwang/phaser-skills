...
4. 原子资源必须绑定当前有效基线和适用分系统锚点。无法绑定时升级 V0 路径，不得凭相似关键词继续生产。
...
禁止直接修改冻结基线。变更提案必须包含：旧/新基线 ID、版本与指纹；变更原因；规则和锚点差异；不变量与允许变量变化；受影响资源、状态、页面、动画、VFX、布局、证据和渠道；迁移/废弃计划；成本、授权与风险；非作者美术结论；按需 F4 决策。

批准新基线后，将旧证据标为失效或限定适用范围，并从最早受影响的 V 阶段重验。至少重验主锚点、受影响分系统锚点、多资源联系表、同屏截图、V4 文件/Phaser 证据和 V5 运行态证据。未完成影响面重验时不得报告视觉完成。
