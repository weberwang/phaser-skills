...
- V3 schema 1.3 资源清单、ownership-first 覆盖区域、来源版本、框选编号、目标尺寸、透明要求、锚点、九宫格、纹理键和预算。
...
4. 效果图提取天然触发条件编号确认：把编号图文件、版本、SHA-256 和决定 ID 写入对应覆盖区域并复算文件哈希；边界改变必须重新确认。
5. 输出独立位图或图集，核对尺寸、锚点、九宫格、采样和纹理预算，再写入唯一权威 `visual-assets.json`。不得把 `runtime-data` 或 `runtime-rendered` 区域裁成位图。
