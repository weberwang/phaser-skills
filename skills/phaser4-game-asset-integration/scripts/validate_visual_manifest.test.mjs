...
async function createFixtureFiles(root, includeAi = false) {
  ...
  for (const path of paths) { const target = join(root, path); await mkdir(dirname(target), { recursive: true }); await writeFile(target, path === "evidence/visual/mockup.png" ? minimalPng(390, 844) : ["public/assets/hero.png", "art/hero-raw.png", "art/hero-cutout.png"].includes(path) ? minimalPng(includeAi ? 96 : 64, includeAi ? 144 : 96) : ""); }
