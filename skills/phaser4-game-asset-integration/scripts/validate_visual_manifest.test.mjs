...
import { loadVisualConfirmationAuthority } from "../../phaser4-game-workflow-control/scripts/visual-confirmation-authority.mjs"; import { computeLayoutAnnotationConfirmationSha256, computeLayoutUserMessageSha256 } from "../../phaser4-game-workflow-control/scripts/layout_annotation_confirmation.mjs"; import { computeLayoutAnnotationIdentitySha256, deriveAutomaticLayoutFacts, renderLayoutAnnotation } from "./layout_annotation_contract.mjs";
...
function addManualConfirmationRecords(manifest) {
  ...
  for (const region of regions) {
    const components = Array.isArray(region.component_inventory?.components) ? region.component_inventory.components : []; region.decomposition_elements = (region.layout_node_ids ?? []).map((layoutNodeId) => { const component = components.find((item) => item?.placements?.some((placement) => placement?.layout_node_id === layoutNodeId)); const placement = component?.placements?.find((item) => item?.layout_node_id === layoutNodeId); return { element_id: layoutNodeId, element_type: "component", role: component?.role ?? region.owner_type ?? "component", bounds: { ...region.bounds }, scene_id: region.scene_id, state_id: region.state_id, region_id: region.id, component_id: component?.component_id ?? `${region.id}-component`, placement_id: placement?.placement_id ?? `${layoutNodeId}-placement`, empty_container: false }; });
...
function attachSceneReconstructionContract(manifest) {
  ...
  const layoutNodes = manifest.coverage_audit.regions.map((region, index) => {
    const bounds = { ...region.bounds }; const parentBounds = { x: 0, y: 0, width: 390, height: 844 }; const left = bounds.x - parentBounds.x; const right = parentBounds.x + parentBounds.width - bounds.x - bounds.width; const top = bounds.y - parentBounds.y; const bottom = parentBounds.y + parentBounds.height - bounds.y - bounds.height;
    /* 测试夹具用预设枚举模拟智能视觉判断，不能通过四边距离反推对齐方向。 */ const horizontal = ["left", "center", "right"][index % 3]; const vertical = ["top", "center", "bottom"][index % 3]; const centerX = bounds.x + bounds.width / 2 - parentBounds.width / 2; const centerY = bounds.y + bounds.height / 2 - parentBounds.height / 2;
    return { layout_node_id: region.layout_node_ids[0], region_id: region.id, scene_id: region.scene_id, state_id: region.state_id, coordinate_space: "viewport", reference_id: "viewport", parent_layout_node_id: "viewport", parent_target_bounds: parentBounds, relative_position: { left, right, top, bottom }, axis_alignment: { horizontal, vertical }, self_anchor: `${vertical}-${horizontal}`, reference_anchor: `${vertical}-${horizontal}`, offset: { x: horizontal === "left" ? left : horizontal === "right" ? -right : centerX, y: vertical === "top" ? top : vertical === "bottom" ? -bottom : centerY }, target_bounds: bounds, size_policy: { mode: "target-bound" }, z_order: index, clip_policy: "none", responsive_rule: "preserve-relative-anchors", planned_test_id: `layout-${region.id}` };
  });
  ...
  manifest.scene_reconstruction_contract = {
    ...
    visual_decomposition_confirmation: { confirmation_id: "v2-confirmation", confirmation_sha256: EMPTY_DOCUMENT_FINGERPRINT, confirmation_mode: "manual", status: "passed", proposal_id: "v2-proposal", proposal_sha256: EMPTY_DOCUMENT_FINGERPRINT, annotation_file: "evidence/v2/decomposition-annotation.png", annotation_sha256: EMPTY_DOCUMENT_FINGERPRINT, target_sha256: targetSha, candidate_identity: { sha256: candidateSha, diff_fingerprint: manifest.candidate_identity.diff_fingerprint } },
    ...
    layout_decomposition: { layout_binding: { target_sha256: targetSha, scene_id: "main-gameplay", state_id: "default", visual_baseline_version: manifest.visual_baseline.version, viewport: { width: 390, height: 844 }, layout_contract_version: "1.1.0", layout_contract_sha256: layoutContractSha, layout_decomposition_version: layoutDecompositionVersion }, layout_nodes: layoutNodes, layout_annotation: { layout_annotation_file: "evidence/v2/layout-annotation.png", layout_annotation_sha256: EMPTY_DOCUMENT_FINGERPRINT, layout_annotation_width: 390, layout_annotation_height: 844, layout_annotation_schema: "layout-annotation/png/1", layout_annotation_layout: "image-plus-right-panel", layout_annotation_metadata_sha256: EMPTY_DOCUMENT_FINGERPRINT, layout_annotation_identity_sha256: EMPTY_DOCUMENT_FINGERPRINT, decomposition_confirmation_id: "v2-confirmation", decomposition_confirmation_sha256: EMPTY_DOCUMENT_FINGERPRINT, proposal_sha256: EMPTY_DOCUMENT_FINGERPRINT, layout_decision_file: "evidence/v2/automatic-layout-decision.json", layout_decision_sha256: EMPTY_DOCUMENT_FINGERPRINT, layout_decision_id: "layout-decision-1", target_sha256: targetSha, scene_id: "main-gameplay", state_id: "default", layout_node_ids: layoutNodes.map((node) => node.layout_node_id) }, layout_annotation_confirmation: (() => { const record = { confirmation_schema: "layout-annotation-confirmation/1.0", confirmation_id: "v2-layout-confirmation", status: "accepted", confirmation_mode: "manual", layout_annotation_file: "evidence/v2/layout-annotation.png", layout_annotation_sha256: EMPTY_DOCUMENT_FINGERPRINT, layout_annotation_width: 390, layout_annotation_height: 844, layout_annotation_schema: "layout-annotation/png/1", layout_annotation_layout: "image-plus-right-panel", layout_annotation_metadata_sha256: EMPTY_DOCUMENT_FINGERPRINT, layout_annotation_identity_sha256: EMPTY_DOCUMENT_FINGERPRINT, decomposition_confirmation_id: "v2-confirmation", decomposition_confirmation_sha256: EMPTY_DOCUMENT_FINGERPRINT, proposal_sha256: EMPTY_DOCUMENT_FINGERPRINT, layout_decision_file: "evidence/v2/automatic-layout-decision.json", layout_decision_sha256: EMPTY_DOCUMENT_FINGERPRINT, layout_decision_id: "layout-decision-1", target_sha256: targetSha, scene_id: "main-gameplay", state_id: "default", user_original_text: "确认布局标注", user_message_sha256: computeLayoutUserMessageSha256("确认布局标注"), decision_record_file: "evidence/v2/layout-decision.json", decision_record_sha256: EMPTY_DOCUMENT_FINGERPRINT, user_decision_receipt_file: "evidence/v2/layout-receipt.json", user_decision_receipt_sha256: EMPTY_DOCUMENT_FINGERPRINT, accepted_at: "2026-01-01T00:00:00Z" }; return { ...record, confirmation_sha256: computeLayoutAnnotationConfirmationSha256(record) }; })() },
    composition: {
...
function validAiManifest() {
  ...
  asset.generation_record = { record_id: "gen-hero-1", generator: "imagegen", generator_version: "1", created_at: "2026-08-15T00:00:00Z", command_or_recipe: "render hero-idle -> background-removal once", input_sources: ["prompt:hero-idle"], parameters: { size: `${width}x${height}` }, reconstruction_mode: "reference-faithful", reference_input_mode: "full-reference-guidance", pixel_reuse_policy: "forbid-output-reuse", source_background_mode: "opaque", final_background_mode: "transparent", transparency_strategy: "background-removal", global_prompt_prefix: EFFECT_IMAGE_GLOBAL_PROMPT_PREFIX, asset_prompt: assetPrompt, state_prompt: statePrompt, negative_prompt: EFFECT_IMAGE_NEGATIVE_PROMPT, full_prompt: fullPrompt, raw_source_file: "art/hero-raw.png", raw_source_has_alpha: false, source_file: "art/hero-cutout.png", source_has_alpha: true, runtime_file: "public/assets/hero.png", output_file: "public/assets/hero.png", background_removal_attempts: [{ operation: "background-removal", status: "completed", source_file: "art/hero-raw.png", output_file: "art/hero-cutout.png", source_has_alpha: false, output_has_alpha: true, completed_at: "2026-08-15T00:00:00Z", evidence: { record_id: "br-hero-1", report: "evidence/visual/hero-background-removal.json" } }], model: "image-model", model_version: "1", seed: 42, reference_inputs: [manifest.reference_target.original_file], style_reference_inputs: ["evidence/visual/ai-reference.png"], postprocess: ["background-removal"], annotation_number: 2, region_id: "region-hero", component_id: "hero-component", state_id: "default", asset_id: "hero-idle", target_sha256: manifest.reference_target.target_sha256, candidate_sha256: manifest.candidate_identity.sha256, diff_fingerprint: manifest.candidate_identity.diff_fingerprint, candidate_version: manifest.candidateVersion, normalization_record: asset.normalization_record };
...
async function createFixtureFiles(root, includeAi = false) {
  ...
  for (const path of paths) { const target = join(root, path); await mkdir(dirname(target), { recursive: true }); await writeFile(target, ["evidence/visual/mockup.png"].includes(path) ? minimalPng(390, 844) : ["public/assets/hero.png", "art/hero-raw.png", "art/hero-cutout.png"].includes(path) ? minimalPng(includeAi ? 96 : 64, includeAi ? 144 : 96) : ""); }
...
async function writeConfirmationFixtureFiles(root, manifest, annotationBytes = null) {
  ...
  for (const [key, groupRegions] of groups) {
    ...
    const technicalRegions = groupRegions.map((region) => ({ ...technicalRegionSnapshot(region), layout_node_ids: [...(region.layout_node_ids ?? [])] }));
    const technicalLayoutNodeIds = groupRegions.flatMap((region) => region.layout_node_ids ?? []); const decompositionElements = groupRegions.flatMap((region) => region.decomposition_elements ?? []);
    const proposal = { schema_version: "1.5", proposal_kind: "effect-image-decomposition-technical-analysis", proposal_id: confirmation.proposal_id, created_at: "2026-08-15T00:15:00Z", target_sha256: manifest.reference_target.target_sha256, scene_id: first.scene_id, state_id: first.state_id, canvas: canvasSnapshot, annotation_file: confirmation.annotation_file, annotation_mime: "image/png", annotation_sha256: annotationSha, decomposition_elements: decompositionElements, regions: regionsSnapshot, visual_regions: visualRegions, technical_analysis: { schema_version: "1", canvas: canvasSnapshot, layout_node_ids: technicalLayoutNodeIds, decomposition_elements: decompositionElements, regions: technicalRegions } };
    ...
    if (manifest.scene_reconstruction_contract?.target_conditions?.scene_id === sceneId && manifest.scene_reconstruction_contract?.target_conditions?.state_id === stateId) manifest.scene_reconstruction_contract.visual_decomposition_confirmation = first.confirmation;
  }
  // 布局阶段夹具必须是真实 PNG/JSON 三件套，文件门不能用空文件模拟人工确认。
  const sceneContract = manifest.scene_reconstruction_contract; const decomposition = sceneContract?.layout_decomposition; const layoutAnnotation = decomposition?.layout_annotation; const layoutConfirmation = decomposition?.layout_annotation_confirmation; const target = sceneContract?.target_conditions; const canvas = manifest.coverage_audit?.canvases?.find((item) => item.scene_id === target?.scene_id && item.state_id === target?.state_id); const upstream = manifest.coverage_audit?.regions?.find((item) => item.scene_id === target?.scene_id && item.state_id === target?.state_id)?.confirmation ?? sceneContract.visual_decomposition_confirmation; if (decomposition && Array.isArray(decomposition.layout_nodes) && layoutAnnotation && layoutConfirmation && target && canvas && upstream) {
    const originalBytes = await readFile(join(root, manifest.reference_target.original_file)); const facts = deriveAutomaticLayoutFacts(decomposition.layout_nodes, canvas, { sceneId: target.scene_id, stateId: target.state_id }); const layoutDecision = { decision_schema: "automatic-layout-decision/1.0", decision_id: layoutAnnotation.layout_decision_id, decision_method: "visual-judgement", target_sha256: manifest.reference_target.target_sha256, scene_id: target.scene_id, state_id: target.state_id, decomposition_confirmation_id: upstream.confirmation_id, decomposition_confirmation_sha256: upstream.confirmation_sha256, proposal_sha256: upstream.proposal_sha256, elements: decomposition.layout_nodes.map((node) => ({ element_id: node.element_id ?? node.layout_node_id, horizontal_alignment: node.axis_alignment.horizontal, vertical_alignment: node.axis_alignment.vertical })) }; const layoutDecisionBytes = Buffer.from(`${JSON.stringify(layoutDecision, null, 2)}\n`); const layoutDecisionPath = join(root, layoutAnnotation.layout_decision_file); await mkdir(dirname(layoutDecisionPath), { recursive: true }); await writeFile(layoutDecisionPath, layoutDecisionBytes); const layoutDecisionSha = sha256Bytes(layoutDecisionBytes); const rendered = renderLayoutAnnotation(originalBytes, canvas, facts, { targetSha256: manifest.reference_target.target_sha256, sceneId: target.scene_id, stateId: target.state_id, decompositionConfirmationId: upstream.confirmation_id, decompositionConfirmationSha256: upstream.confirmation_sha256, decompositionProposalSha256: upstream.proposal_sha256, layoutDecisionId: layoutDecision.decision_id, layoutDecisionSha256: layoutDecisionSha }); const layoutSha = sha256Bytes(rendered.bytes); const layoutPath = join(root, layoutAnnotation.layout_annotation_file); await writeFile(layoutPath, rendered.bytes); Object.assign(layoutAnnotation, { layout_annotation_sha256: layoutSha, layout_annotation_width: rendered.width, layout_annotation_height: rendered.height, layout_annotation_schema: rendered.metadata.schema, layout_annotation_layout: rendered.metadata.layout, layout_annotation_metadata_sha256: rendered.metadataSha256, layout_annotation_identity_sha256: computeLayoutAnnotationIdentitySha256(layoutSha, rendered.width, rendered.height, rendered.metadataSha256), decomposition_confirmation_id: upstream.confirmation_id, decomposition_confirmation_sha256: upstream.confirmation_sha256, proposal_sha256: upstream.proposal_sha256, layout_decision_sha256: layoutDecisionSha, target_sha256: manifest.reference_target.target_sha256, scene_id: target.scene_id, state_id: target.state_id, layout_node_ids: decomposition.layout_nodes.map((node) => node.layout_node_id) });
    const userText = layoutConfirmation.user_original_text; const binding = { layout_annotation_file: layoutAnnotation.layout_annotation_file, layout_annotation_sha256: layoutSha, layout_annotation_width: rendered.width, layout_annotation_height: rendered.height, layout_annotation_schema: rendered.metadata.schema, layout_annotation_layout: rendered.metadata.layout, layout_annotation_metadata_sha256: rendered.metadataSha256, layout_annotation_identity_sha256: layoutAnnotation.layout_annotation_identity_sha256, decomposition_confirmation_id: layoutAnnotation.decomposition_confirmation_id, decomposition_confirmation_sha256: layoutAnnotation.decomposition_confirmation_sha256, proposal_sha256: layoutAnnotation.proposal_sha256, layout_decision_file: layoutAnnotation.layout_decision_file, layout_decision_sha256: layoutAnnotation.layout_decision_sha256, layout_decision_id: layoutAnnotation.layout_decision_id, target_sha256: layoutAnnotation.target_sha256, scene_id: layoutAnnotation.scene_id, state_id: layoutAnnotation.state_id, user_statement: userText, user_message_sha256: computeLayoutUserMessageSha256(userText), accepted_at: layoutConfirmation.accepted_at }; const decision = { author_role: "user", resolution_status: "resolved", resolved_from: "USER_INPUT_REQUIRED", status: "accepted", confirmation_mode: "manual", confirmation_id: layoutConfirmation.confirmation_id, ...binding }; const decisionPath = join(root, layoutConfirmation.decision_record_file); await mkdir(dirname(decisionPath), { recursive: true }); await writeFile(decisionPath, `${JSON.stringify(decision, null, 2)}\n`); const decisionSha = sha256Bytes(Buffer.from(`${JSON.stringify(decision, null, 2)}\n`)); const receipt = { message_id: `message-${layoutConfirmation.confirmation_id}`, thread_id: "thread-layout", resolution_id: `resolution-${layoutConfirmation.confirmation_id}`, author_role: "user", resolution_status: "resolved", resolved_from: "USER_INPUT_REQUIRED", ...binding, decision_record_sha256: decisionSha }; const receiptBytes = Buffer.from(`${JSON.stringify(receipt, null, 2)}\n`); const receiptPath = join(root, layoutConfirmation.user_decision_receipt_file); await mkdir(dirname(receiptPath), { recursive: true }); await writeFile(receiptPath, receiptBytes); const receiptSha = sha256Bytes(receiptBytes); Object.assign(layoutConfirmation, { ...binding, decision_record_sha256: decisionSha, user_decision_receipt_sha256: receiptSha }); layoutConfirmation.confirmation_sha256 = computeLayoutAnnotationConfirmationSha256(layoutConfirmation); }}
