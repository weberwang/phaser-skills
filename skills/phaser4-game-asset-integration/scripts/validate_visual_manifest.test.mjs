...
async function writeConfirmationFixtureFiles(root, manifest, annotationBytes = null) {
  ...
  for (const [key, groupRegions] of groups) {
    ...
    const technicalLayoutNodeIds = groupRegions.flatMap((region) => region.layout_node_ids ?? []).sort(); const decompositionElements = groupRegions.flatMap((region) => region.decomposition_elements ?? []).sort((left, right) => left.element_id.localeCompare(right.element_id));
