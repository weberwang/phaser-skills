...
function attachSceneReconstructionContract(manifest) {
  ...
  const layoutNodes = manifest.coverage_audit.regions.map((region, index) => {
    ...
    const component = region.component_inventory?.components?.[0]; const placement = component?.placements?.[0]; return { layout_node_id: region.layout_node_ids[0], element_id: region.layout_node_ids[0], element_type: "component", region_id: region.id, scene_id: region.scene_id, state_id: region.state_id, component_id: component?.component_id ?? `${region.id}-component`, placement_id: placement?.placement_id ?? `${region.layout_node_ids[0]}-placement`, parent_element_id: "viewport", semantic_grouping: { kind: "standalone", rationale: "测试夹具中的完整视觉元素独立布局" }, empty_container: false, coordinate_space: "viewport", reference_id: "viewport", parent_layout_node_id: "viewport", parent_target_bounds: parentBounds, relative_position: { left, right, top, bottom }, axis_alignment: { horizontal, vertical }, self_anchor: `${vertical}-${horizontal}`, reference_anchor: `${vertical}-${horizontal}`, offset: { x: horizontal === "left" ? left : horizontal === "right" ? -right : centerX, y: vertical === "top" ? top : vertical === "bottom" ? -bottom : centerY }, target_bounds: bounds, size_policy: { mode: "target-bound" }, z_order: index, clip_policy: "none", responsive_rule: "preserve-relative-anchors", planned_test_id: `layout-${region.id}` };
  });
...
function validAiManifest() {
  ...
  Object.assign(manifest.coverage_audit.regions[1], { production_origin: "independent-production", production_method: "image-generation", delivery_kind: "raster-image", image_generation_required: true, generation_record_required: true, substitution_policy: "user-change-request-only" });
  ...
  sceneHeroRegion.production_method = "image-generation";
...
function multiConfirmationGroupManifest() {
  ...
  second.state_id = "victory"; /* 当前布局只绑定 default 提案，迁出的胜利态元素不得留在其中。 */ manifest.scene_reconstruction_contract.layout_decomposition.layout_nodes = manifest.scene_reconstruction_contract.layout_decomposition.layout_nodes.filter((node) => node.region_id !== second.id);
...
async function writeConfirmationFixtureFiles(root, manifest, annotationBytes = null) {
  ...
  // 布局阶段夹具必须是真实 PNG、节点 JSON、决策副本和离线审阅页，文件门不能用空文件模拟人工确认。
  await writeLayoutReviewFixtureArtifacts(root, manifest);
}
...
test("拆解资产只有显式 图像生成 才要求 AI 路线且独立生产不得直接复用冻结原图", () => { const bitmap = validManifest(); const region = bitmap.coverage_audit.regions[1]; region.production_origin = "bitmap-decomposition"; region.production_method = "image-generation"; region.delivery_kind = "raster-image"; region.image_generation_required = true; region.generation_record_required = true; const asset = bitmap.assets[0]; asset.production_origin = "bitmap-decomposition"; asset.production_method = "image-generation"; asset.delivery_kind = "raster-image"; asset.image_generation_required = true; asset.generation_record_required = true; assert(validateManifest(bitmap).some((item) => item.includes("route 必须为 ai-composite-raster"))); const independent = validManifest(); independent.assets[0].source_file = independent.reference_target.original_file; assert(validateManifest(independent).some((item) => item.includes("不得直接把冻结效果图"))); });
...
test("每个 scene/state 组拥有独立 PNG、提案和决定文件并完整覆盖组内编号", async () => {
  ...
  assert.equal(manifest.scene_reconstruction_contract.layout_decomposition.layout_nodes.length, 2); assert.deepEqual(await checkManifestFiles(manifest, root), []);
});
