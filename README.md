...
游戏实现不是可省略的角色调用，而是贯穿 G1/G2 的闭环。参考截图、录屏、运行项目或源代码还原任务必须先冻结逐状态功能契约和视觉基线，再按“功能与结构低保真 → 人工确认 → 高保真效果图与确认 → 效果图审阅 → 框出全部正式单图并确认 → 单图生成与验收 → 功能与结构化装配 → 双验收后删除低保真实现”执行。纯规则编码可在 A1 确认后并行，但不得跳过或倒置任何确认门；功能通过不能替代视觉通过，视觉接近也不能替代功能复现。
