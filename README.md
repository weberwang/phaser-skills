...
- `$grilling`：一般只在证据无法关闭且确需人工取舍时处理受保护决策；新模块首次实现、模块边界变化，以及适用 V2 的当前高保真效果图出口确认属于强制例外。
...
审核漏斗为 F0 入口、F1 证据、F2 非作者专业、F3 综合、按需 F4 人工。F2 不能由作者自审；缺少第二角色时由总控代审并标识。F3 只用于集成与跨域风险，F4 只处理范围、方向、预算/授权风险、风险接受和发布放行等受保护决策；适用 V2 的当前高保真效果图用户确认是视觉路径的强制受保护决策例外，且不能豁免专业缺陷或缺证。
...
一般 `$grilling` 只处理受保护决策；但新模块首次实现或模块边界变化时，无论现有证据是否完整，都必须完成模块拷问并取得人工批准。视觉路径中，V2b 与独立美术 F2 通过后还必须由总控通过 `$grilling` 向用户提交当前候选版本的高保真效果图确认包；只有明确“通过”才能形成视觉可交付结论并进入 V3。两项强制例外都不能接收仍应由实现、证据或专业审阅关闭的问题。只有在 TDD 记录模块清单、依赖矩阵和确认记录，且状态为“已批准待实现”后，才能开始正式实现。

## V0-V5 视觉生产

V0 先分流：

- 原子资源：视觉方向已冻结、结构/交互/布局不变，并且已有适用、有效、绑定版本且覆盖该范围的玩法视觉契约、高保真效果图确认记录、视觉可交付结论与预算基线可引用的单个或小批同类正式资源，走 V3→V4→V5；F0 一次、F1 逐包、V4 做 F2，V5 做集成 F1 和动态证据，只有跨域风险才 F3。缺少任一可引用基线或确认记录时升级为组件/资源集或场景路径；重做类任务不得按原子资源跳过 V2。
- 组件/资源集：走 V1→V5；候选定义或改变用户可见高保真形态时执行 V2a/V2b 与 V2 出口确认，V4 做 F2，V5 做 F3。
- 场景、整套 UI、视觉系统、参考还原，以及所有重做、重新设计、提升游戏感或替换整套 UI 的任务：走完整 V1→V5；必须执行 V2a/V2b、V2 出口高保真效果图确认，V3/V4 做 F2，V5 做 F3。

V1 建立逐状态玩法视觉契约、全局视觉基线候选、必要灰盒和早期预算；V2 依次完成 V2a 方向基准、冻结单一版本化全局视觉基线、V2b 整体视觉审阅与动态可玩样片、独立美术 F2，再由总控提交当前候选版本的高保真效果图确认包。用户明确“通过”后才形成绑定确认记录的视觉可交付结论并进入 V3；“修改后重提”产生新版本并重做受影响 V2b/F2，“拒绝”按根因退 V2a 或 V1。V3 按 UI、像素美术、逐帧/骨骼动画、Tilemap、VFX、装饰背景、玩法环境或 AI 合成栅格等路线设计可编辑源文件、运行时输出和机器清单，并将每个资源绑定基线 ID、版本与风格指纹；V4 生产正式资源并以联系表和同屏截图做跨资源一致性验收；V5 结构化集成、运行态一致性、动态玩法视觉验收和低保真清理。

视觉完成按实际路径检查适用 V 阶段、适用 F2、适用时 F3、通常按需 F4 及强制的 V2 出口确认例外，并要求绑定当前高保真确认记录的视觉可交付结论与 V4/V5 的工程可交付结论同时有效。沉默、继续工作、旧版本批准或 V2a 方向级批准均不构成当前候选确认。原子资源可以引用既有 V1 契约、适用有效且绑定版本的高保真确认记录、视觉可交付结论与预算，不为形式重跑 V1，也不因进入 V5 自动要求 F3。
